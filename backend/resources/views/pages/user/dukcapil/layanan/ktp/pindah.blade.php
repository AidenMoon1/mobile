...
@section('title', 'KTP Perpindahan - Sukabumi One Access')
...
<section class="form-header">
    ...
    <div class="max-w-4xl mx-auto relative z-10 text-center text-white">
        <div class="reveal">
            ...
            <p class="text-slate-400 mt-2 text-sm">Penerbitan KTP-el karena Perpindahan Domisili</p>
        </div>
    </div>
</section>
...
<section class="pb-24 bg-slate-50 min-h-screen">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 form-container">
        ...
        <form action="{{ route('layanan.ktp.pindah.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="bg-white rounded-[2.5rem] p-6 sm:p-10 shadow-xl shadow-slate-200/50 reveal">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="input-group-custom md:col-span-2">
                        <label>Alasan Pindah (Opsional)</label>
                        <textarea name="keterangan" rows="2" placeholder="Contoh: Mengikuti keluarga / Pekerjaan..."></textarea>
                    </div>
                </div>
            </div>

            {{-- CARD 2: ALAMAT TUJUAN (ALAMAT BARU) --}}
            <div class="bg-white rounded-[2.5rem] p-6 sm:p-10 shadow-xl shadow-slate-200/50 reveal">
                <div class="flex items-center gap-4 mb-8">
                    <div class="w-10 h-10 rounded-xl bg-accent text-primary flex items-center justify-center shadow-lg">
                        <i class="bi bi-geo-alt-fill"></i>
                    </div>
                    <h3 class="text-lg font-bold text-primary">Alamat Tujuan (Kota Sukabumi)</h3>
                </div>
            ...
            <div class="bg-white rounded-[2.5rem] p-6 sm:p-10 shadow-xl shadow-slate-200/50 reveal">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- File 1: SKPWNI --}}
                    <div class="space-y-3">
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1 text-center sm:text-left">Foto KK Baru Karena Perpindahan</p>
                        <div class="upload-box" onclick="document.getElementById('file_skpwni').click()">
                            <input type="file" id="file_skpwni" name="file_skpwni" accept="image/*" class="hidden" onchange="previewFile(this, 1)">
                            <div id="placeholder1">
                                <i class="bi bi-file-earmark-arrow-up text-3xl text-slate-300 block mb-2"></i>
                                <span class="text-[10px] font-bold text-slate-400 uppercase">Pilih Foto Keluarga</span>
                            </div>
                            <!-- Wrapper Preview Baru -->
                            <div id="previewWrap1" class="preview-wrap mx-auto">
                                <img id="preview1" class="preview-img">
                                <button type="button" class="remove-preview-btn" onclick="removePreview(event, 'file_skpwni', 1)">
                    ...
                    {{-- File 2: Pendukung --}}
                    <div class="space-y-3">
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1 text-center sm:text-left">Foto Pendukung Lainnya</p>
                        <div class="upload-box" onclick="document.getElementById('file_kk').click()">
                            ...
                            <div id="placeholder2">
                                ...
                                <span class="text-[10px] font-bold text-slate-400 uppercase">Pilih Foto Pendukung</span>
                            </div>
                            <!-- Wrapper Preview Baru -->
                            <div id="previewWrap2" class="preview-wrap mx-auto">
                                <img id="preview2" class="preview-img">
                                <button type="button" class="remove-preview-btn" onclick="removePreview(event, 'file_kk', 2)">
                                    <i class="bi bi-trash3-fill"></i>
                                </button>
                            </div>
                        </div>
...
<script>
    ...
    function removePreview(event, inputId, index) {
        event.stopPropagation(); // Penting: Mencegah trigger klik pada upload-box
        ...
        previewWrap.style.display = 'none'; // Sembunyikan wrapper
        placeholder.style.display = 'block'; // Munculkan placeholder lagi
    }

    // 4. API WILAYAH INDONESIA (emsifa) - VERSI SINKRONISASI NAMA
