...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // ---------------------------------------------------------------------------
  // 2. MODAL OTENTIKASI SUKABUMI SINGLE SIGN-ON (SSO PEMKOT)
  // ---------------------------------------------------------------------------
  void _bukaModalSukabumiSSO() {
    final TextEditingController ssoUserController = TextEditingController(text: 'sso.warga@sukabumi.go.id');
    bool isAuthenticating = false;
          ...
          return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.vpn_key_rounded, color: Color(0xFFE8A33D), size: 28),
                    ),
                    const SizedBox(width: 14),
                    const Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Layanan Satu Akun Terpadu Pemkot Sukabumi',
                            style: TextStyle(fontSize: 11, color: Colors.grey, fontFamily: 'Poppins'),
                          ),
                        ],
                      ),
                ...
                const SizedBox(height: 20),
                const Text(
                  'Masuk menggunakan Akun SSO Terpadu Pemkot Sukabumi untuk mengakses seluruh portal layanan publik:',
                  style: TextStyle(fontSize: 12, color: Colors.black87, fontFamily: 'Poppins'),
                ),
                const SizedBox(height: 14),
                TextFormField(
                  controller: ssoUserController,
                  style: const TextStyle(fontSize: 13, fontFamily: 'Poppins'),
                  decoration: InputDecoration(
                    labelText: 'Username SSO / Email / NIP',
                    hintText: 'sso.warga@sukabumi.go.id',
                    prefixIcon: const Icon(Icons.person_pin_rounded, color: Color(0xFFE8A33D)),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                    filled: true,
                    fillColor: const Color(0xFFF8FAFC),
                  ),
                  ...
                  child: ElevatedButton.icon(
                    onPressed: isAuthenticating
                        ...
                        : () async {
                            ...
                            final ssoUser = ssoUserController.text.trim();
                            final current = UserService().currentUser;

                            await UserService().updateProfile(
                              current.copyWith(
                                username: ssoUser.isNotEmpty ? ssoUser : 'sso.warga',
                                name: 'Warga Sukabumi (SSO Verified)',
                                status: 'Terverifikasi (SSO Pemkot)',
                              ),
                            );

                            await NotificationService().addNotification(
                              title: '🔑 Login SSO Sukabumi Berhasil',
                              description: 'Otentikasi akun Sukabumi Single Sign-On ($ssoUser) berhasil.',
                              category: NotificationCategory.general,
                            );
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('Login Sukabumi SSO Berhasil! Welcome Warga Sukabumi.'),
                                backgroundColor: Color(0xFF0A1E33),
                              ),
                            );
                    ...
                    label: Text(
                      isAuthenticating ? 'Menghubungkan Server SSO...' : 'Masuk Dengan Akun SSO',
                      style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, fontFamily: 'Poppins'),
                    ),
                    style: ElevatedButton.styleFrom(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!_isAdminMode) ...[
                        ...
                        // 2. TOMBOL SUKABUMI SINGLE SIGN-ON (SSO PEMKOT)
                        SizedBox(
                          width: double.infinity,
                          height: 48,
                          child: ElevatedButton.icon(
                            onPressed: _bukaModalSukabumiSSO,
